...
while (running === 1) {
  ...
  const choices = {
    ...
    fish: function () {
      ...
      //First step in the 'catch a fish' chain. If yes, we move to the chuck function.
      return keep === "N" || keep === "n"
        ? (onRod = [])
        : keep === "Y" || keep === "y"
        ? choices.chuck(newFish)
        : "";
    },
    ...
    cast: function () {
      ...
      return a <= 20
        ? prompt(
            "You fished up some junk. It is not worth anything. Press ENTER to continue."
          )
        : choices.fish();
    },
    ...
    thisThat: function (a) {
      //To start off, we make a copy of the newFish and push it to the bucket.
      aAA = a;
      bucket.push(aAA);
      // console.log('You have made it to your thisThat function with', a)
      //     prompt ('>');

      //Sum of fish. The value here will be over 10, as we have added the weight of the newFish to bucket.
      let sumWeight = 0;
      for (let i = 0; i < bucket.length; i++) {
        sumWeight += Number(bucket[i].weight);
      }
      ...
      //This loop will list out to the player, every fish in their bucket (inlcuding the newFish)
      //along with all data stored in them. The player MUST remove a fish here, there is no other
      //way to escape the loop.
      for (let i = 0; i < bucket.length; i++) {
        console.log(
          "#",
          i + 1,
  ...
  console.log(
    ...
    "It is currently",
    time,
    "and a gorgeous sunny day outside.",
    "\n",
    "\n",
  ...
  function checkAction(a) {
    ...
    for (const action in choices) {
      if (a === "fish" || a === "thisThat") {
        return checkAction(prompt(">").toLowerCase());
      }
      a === action ? aA++ : "";
    }
...
if (running === 3) {
  console.clear();
  console.log(
    "It is noon and you are STARVING! Maybe that's enough fishing for the day."
  );
  prompt(">");
  ...
  console.log(
    "You caught",
    sumFish,
    "fish today, they weigh-in at",
    sumWeight,
    ".",
    "\n",
    "After some haggling, you sell the lot for $",
    sumCash,
    ", nice job!"
  );
  prompt(">");
